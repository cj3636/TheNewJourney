...
public class FireTerrainGenerator {
    private World world;
    ...
    public void setBiomesForGeneration(Biome[] biomesForGeneration2) {
        this.biomesForGeneration = new Biome[]{Biomes.HELL};
    }
                ...
                for (int height32 = 0; height32 < 32; ++height32) {
                    ...
                    for (int h = 0; h < 8; ++h) {
                        ...
                        for (int x = 0; x < 4; ++x) {
                            ...
                            for (int z = 0; z < 4; ++z) {
                                if (height < 2) {
                                    primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, ModBlocks.BurntStone.getDefaultState());
    ...
    public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, IChunkGenerator generator, Biome[] biomes) {
        ...
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                ...
                this.generateBiomeTerrain(this.world, this.random, primer, x * 16 + i, z * 16 + j, this.depthBuffer[j + i * 16]);
            }
        }
    }

    public IBlockState topBlock = ModBlocks.BurntStone.getDefaultState();
    public IBlockState fillerBlock = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState STONE = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState AIR = Blocks.AIR.getDefaultState();
    protected static final IBlockState BEDROCK = ModBlocks.BurntStone.getDefaultState();
    protected static final IBlockState GRAVEL = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState RED_SANDSTONE = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState SANDSTONE = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState ICE = Blocks.FLOWING_LAVA.getDefaultState();
    protected static final IBlockState WATER = Blocks.FLOWING_LAVA.getDefaultState();
        ...
        for (int j1 = 255; j1 >= 0; --j1) {
            if (j1 <= rand.nextInt(5)) {
                ...
                if (iblockstate2.getMaterial() == Material.AIR) {
                    j = -1;
                } else if (iblockstate2.getBlock() == ModBlocks.BurntStone) {
                    if (j == -1) {
                        if (k <= 0) {
                            iblockstate = AIR;
                            iblockstate1 = STONE;
