...
public class BlastFurnaceTileEntity extends TileEntity implements ISidedInventory, ITickable {
    public static BlastFurnaceTileEntity instance = new BlastFurnaceTileEntity();
    // Create and initialize the itemStacks variable that will store store the itemStacks
    public static final int FUEL_SLOTS_COUNT = 2;
     ...
     */
    private static final short COOK_TIME_FOR_COMPLETION = 1200;  // vanilla value is 200 = 10 seconds

    private int cachedNumberOfBurningSlots = -1;

    private static boolean vanillaOre;
    ...
    private int burnFuel() {
        ...
        for (int i = 0; i < FUEL_SLOTS_COUNT; i++) {
            ...
            if (burnTimeRemaining[fuelSlotNumber] > 0) {
                --burnTimeRemaining[fuelSlotNumber];
                ++burningCount;
            ...
            if (burnTimeRemaining[fuelSlotNumber] == 0) {
                if (itemStacks[fuelSlotNumber] != null && getItemBurnTime(itemStacks[fuelSlotNumber]) > 0) {
                    ...
                    burnTimeRemaining[fuelSlotNumber] = burnTimeInitialValue[fuelSlotNumber] = getItemBurnTime(itemStacks[fuelSlotNumber]);
    ...
    private boolean smeltItem(boolean performSmelt) {
        ...
        if (itemStacks[firstSuitableOutputSlot] == ItemStack.EMPTY) {
            ...
            int resultNum = itemStacks[firstSuitableOutputSlot].getCount() + result.getCount();
            itemStacks[firstSuitableOutputSlot].setCount(resultNum);
        }
        markDirty();
        return true;
    }
...
//		// Save the stored item stacks
    ...
    // The field ID is limited to byte, and the field value is limited to short. (if you use more than this, they get arcane down
