...
public class FireRuin extends WorldGenerator {
    ...
    public boolean generate_r0(World world, Random rand, BlockPos pos) {
        ...
        Block blk2 = ModBlocks.FireOre;
        Block blk3 = ModBlocks.CrypticBlock;
        Block blk4 = ModBlocks.DistortedBlock;
        Block blk5 = ModBlocks.FireBlock;
        Block blk6 = Blocks.FIRE;
        Block blk7 = Blocks.CHEST;
        ...
        IBlockState stateFire = blk5.getDefaultState();
        ...
        if (rand.nextFloat() < .12) {
            if (!world.isRemote) {
                ...
                ResourceLocation loc = new ResourceLocation(Ref.MODID, "Tower");
                ...
                if (template != null) {
                    ...
                    template.addBlocksToWorld(world, pos.up(50), placementsettings);
                }
                for (EntityPlayer playerEntity : world.playerEntities) {
                    playerEntity.sendMessage(new TextComponentTranslation(TextFormatting.AQUA + "Mystic whispers shroud you with wisdom..."));
                }
                TileEntityChest entityChest = (TileEntityChest) world.getTileEntity(pos64);

                if (entityChest != null) {
                    if (rand.nextInt(3) == 0) {
                        entityChest.setInventorySlotContents(1, new ItemStack(ModItems.Quazar, 1));
                    }
                    if (rand.nextInt(3) == 1) {
                        entityChest.setInventorySlotContents(2, new ItemStack(ModItems.Saronade, 1));
                    }
                    if (rand.nextInt(3) == 2) {
                        entityChest.setInventorySlotContents(3, new ItemStack(ModItems.Time, 1));
                    }
                }
            }
        }
        if (!world.isRemote) {
            ...
            if (entityChest != null) {
                entityChest.setInventorySlotContents(13, new ItemStack(ModItems.AquisIngot, 1));
                if (rand.nextFloat() < .12) {
                    entityChest.setInventorySlotContents(0, new ItemStack(ModItems.ArcaneArtifact, 1));
                }
            }
            for (EntityPlayer playerEntity : world.playerEntities) {
                playerEntity.sendMessage(new TextComponentTranslation("\u00A75" + "\u00A7o" + "The ground shakes...You feel a dark presence erupt around you."));
