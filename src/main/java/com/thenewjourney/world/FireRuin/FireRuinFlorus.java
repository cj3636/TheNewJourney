...
public class FireRuinFlorus extends WorldGenerator {
    private int xPos;
    private int yPos;
    private int zPos;
    ...
    public boolean generate_r0(World world, Random rand, BlockPos pos) {
        ...
        IBlockState stateFire = blk5.getDefaultState().withProperty(VisceonFlorusPortal.AXIS, EnumFacing.Axis.Y);
        ...
        if (rand.nextFloat() < .12) {
            if (!world.isRemote) {
                ...
                ResourceLocation loc = new ResourceLocation(Ref.MODID, "Ship");
                ...
                if (template != null) {
                    ...
                    template.addBlocksToWorld(world, pos.up(25), placementsettings);
                }
                for (EntityPlayer playerEntity : world.playerEntities) {
                    playerEntity.sendMessage(new TextComponentTranslation(TextFormatting.AQUA + "A loud whoosh draws your attention..."));
                }
            }
        }
        if (!world.isRemote) {
